...
Aplicação em Streamlit para acompanhar o curso **Os 4D's do Negócio** usando a
API oficial do Google Classroom em modo somente leitura. O painel mostra
entregas, atrasos, taxa por etapa e sinais objetivos de atenção.

Este README também é o manual de continuidade do projeto. Ele explica como o
sistema funciona, o papel de cada arquivo, como reconstruir o ambiente em outro
computador e quais informações nunca devem ser enviadas ao Git.
...
> secret no código, no README, em issues, commits ou logs compartilhados.
...
| --- | --- |
| `README.md` | Manual operacional, de reconstrução e manutenção. |
...
O projeto foi validado com Python 3.12. Crie o ambiente e instale exatamente as
versões registradas:
...
instalado no sistema. Não copie `.venv` de outro computador; recrie-o.
